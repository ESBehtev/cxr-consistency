...
DEFAULT_CONFIG_ROOT = ROOT / "configs" / "text_sweeps"
DEFAULT_OUTPUT_ROOT = ROOT / "experiments" / "text_sweeps"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Queue launcher for controlled text encoder sweeps.")
    ...
    parser.add_argument("--family", choices=["convnext", "vit", "deit"], action="append")
...
def discover_configs(args: argparse.Namespace) -> list[Path]:
    ...
    for path in paths:
        path = path.resolve()
        cfg = read_yaml(path)
        if families and infer_family(path, cfg) not in families:
            continue
        out.append(path)
    return out
...
def build_jobs(config_paths: list[Path]) -> list[Job]:
    ...
    for path in config_paths:
        ...
        jobs.append(Job(
            config_path=path,
            run_name=run_name,
            family=infer_family(path, cfg),
            image_encoder=str(cfg.get("image_encoder_name") or "unknown"),
            text_encoder=str(cfg.get("text_encoder_name") or "unknown"),
            freeze_text_encoder=bool(cfg.get("freeze_text_encoder", False)),
            run_dir=run_dir,
            expected_vram_mb=int(resources.get("expected_vram_mb", 10000)),
        ))
    return jobs
...
def job_payload(job: Job) -> dict:
    return {
        ...
        "freeze_text_encoder": job.freeze_text_encoder,
...
def write_status(jobs: list[Job], output_root: Path, event: dict | None = None) -> None:
    payload = {
        "updated_at": now(),
        "counts": {s: sum(1 for job in jobs if job.status == s) for s in ["PENDING", "RUNNING", "OK", "COLLAPSED", "FAILED", "SKIPPED"]},
        "jobs": [job_payload(job) for job in jobs],
    }
    write_json(output_root / "text_sweep_status.json", payload)
    if event is not None:
        append_jsonl(output_root / "text_sweep_status.jsonl", {"time": now(), **event})
...
def can_launch(job: Job, running: list[Job], args: argparse.Namespace, free_mb: int | None, total_mb: int | None) -> bool:
    if len(running) >= args.max_parallel_jobs:
        return False
    projected = sum(r.expected_vram_mb for r in running) + job.expected_vram_mb
...
def print_table(jobs: list[Job], free_mb: int | None) -> None:
    ...
    print("run_name                                family    image          text           freeze status     pid")
    for job in jobs:
        pid = job.process.pid if job.process else "-"
        print(f"{job.run_name[:38]:38} {job.family[:9]:9} {job.image_encoder[:14]:14} {job.text_encoder[:14]:14} {str(job.freeze_text_encoder):6} {job.status[:10]:10} {pid}")
...
def main() -> None:
    ...
    if not jobs:
        raise SystemExit("No text sweep configs found")
    for job in jobs:
        preflight(job, args.rerun_completed)
    if args.dry_run:
        print(f"Text sweep dry-run: jobs={len(jobs)} max_parallel_jobs={args.max_parallel_jobs}")
        for job in jobs:
            print(f"DRY {job.status:10} {job.family:8} {job.image_encoder:14} {job.text_encoder:14} freeze={job.freeze_text_encoder} run={job.run_name} expected_vram_mb={job.expected_vram_mb}")
    ...
    print("Text sweep finished", flush=True)
